...
public class Parabole : MonoBehaviour
{
    ...
    public void OnTouchEnded()
    {
        ...
        PlayerInput input = new PlayerInput
        {
            ...
            //number = CompareTag("Player") && GetComponent<Minion>().currentBomb == null ? GetComponent<Minion>().minionNumber :  (short)- GetComponent<Bomb>().minionNumber
        };


        if (CompareTag("Player"))//拖动的是人
        {
            Minion minion = GetComponent<Minion>();

            if(minion.currentBomb == null)
            {
                input.number = minion.minionNumber;
            }
            else//生成了炸弹 却误点到了人
            {
                input.number = (short)-minion.currentBomb.minionNumber;
            }
        ...
        {
            input.number = (short)-GetComponent<Bomb>().minionNumber;
        }
